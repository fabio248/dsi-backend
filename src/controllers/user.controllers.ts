...
export const createUserWithPet = async (
  ...
  try {
    ...
    const user = await userService.create({ ...input, pet: undefined });

    const pet = await petService.create({ ...inputPet, user: user.id });

    res
      .status(201)
      .json({ message: 'entities created', data: { ...user, pet } });
  } catch (error) {
    next(error);
  }
};
